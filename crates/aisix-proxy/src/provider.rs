...
fn azure_auth_and_base_url(
    config: &aisix_llm::providers::configs::AzureProviderConfig,
    ...
    let api_version = config
        .api_version
        .as_deref()
        .unwrap_or(aisix_llm::providers::azure::DEFAULT_API_VERSION);
...
fn bedrock_auth_and_base_url(
    config: &aisix_llm::providers::configs::BedrockProviderConfig,
) -> Result<(ProviderAuth, Option<Url>)> {
...
mod tests {
    ...
    use aisix_llm::providers::configs::{
    ...
    fn provider_auth_and_base_url_rejects_invalid_bedrock_endpoint_scheme() {
        ...
        assert_matches!(
            error,
            aisix_llm::error::GatewayError::Internal(message)
